...
class PaddleNet(paddle.nn.Layer):
    ...
    def __call__(self, input_tensor):
        ...
        conv02_out, _, _ = self.conv2(act01_out)
        return conv01_out, norm01_out, act01_out, conv02_out




if __name__ == '__main__':
    # 测试L2Decay()、卷积层参数学习率设为0.3和2.0（默认都是1.0）、Momentum优化算法。动态图版。
    use_gpu = False
    ...
    # optimizer
    optim_args = dict(momentum=0.9, use_nesterov=False)
    ...
    for step in range(8):
        ...
        paddle_conv01_out, paddle_bn01_out, paddle_act01_out, paddle_conv02_out = model(batch_data2)
        ...
        paddle_conv01_out, paddle_bn01_out, paddle_act01_out, paddle_conv02_out = model(test_data2)
        paddle_test_bn01_out = paddle_bn01_out.numpy()
        paddle_test_conv02_out = paddle_conv02_out.numpy()
        model.train()
        # 自己网络的test
        print('\ntest_forward:')
