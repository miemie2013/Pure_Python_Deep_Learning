...
class CBatchNorm2d(torch.nn.Module):
    ...
    def forward(self, input, weight):
        ...
        y = input.transpose(0, 1)   # [C, N, H, W]
        return_shape = y.shape      # [C, N, H, W]
        y = y.contiguous().view(input.size(1), -1)   # [C, N*H*W]
        ...
        if self.buffer_num > 0 and self.training and input.requires_grad:  # some layers are frozen!
            ...
            dmudw = torch.autograd.grad(
                outputs=cur_mu.sum(), inputs=weight,
                create_graph=False, retain_graph=True, only_inputs=True
            )[0]
            dmeanx2dw = torch.autograd.grad(
                outputs=cur_meanx2.sum(), inputs=weight,
                create_graph=False, retain_graph=True, only_inputs=True
            )[0]
...
class Conv2dUnit(torch.nn.Module):
    def __init__(self,
        ...
        if cbn:
            self.cbn = CBatchNorm2d(filters, buffer_num=3, burnin=8, out_p=True)
    ...
    def forward(self, x):
        ...
        if self.bn:
            ...
            norm_out = self.af(conv_out)
        elif self.cbn:
            if self.use_dcn:
                norm_out = self.cbn(conv_out, self.conv.dcn_weight)
            else:
