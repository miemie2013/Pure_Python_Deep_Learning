...
if __name__ == '__main__':
    ...
    with fluid.program_guard(train_prog, startup_prog):
        with fluid.unique_name.guard():
            ...
            act02_out_tensor = fluid.layers.sigmoid(fc02_out_tensor)
    ...
    with fluid.program_guard(eval_prog, startup_prog):
        with fluid.unique_name.guard():
            ...
            fc02_out_tensor = fluid.layers.fc(input=act01_out_tensor, size=8,
                                                    ...
                                                    bias_attr=ParamAttr(name="fc02_bias"))
            act02_out_tensor = fluid.layers.sigmoid(fc02_out_tensor)
            eval_fetch_list = [act01_out_tensor, act02_out_tensor]
    ...
    for step in range(8):
        ...
        aa = np.maximum(0, batch_data)
        aa1 = (batch_data > 0).astype(np.float32)
        print()

        paddle_mseloss_out, paddle_act01_out, paddle_act02_out = exe.run(train_prog, feed={"input_1": batch_data, "y_true": y_true_arr, },
                                                                       fetch_list=[mseloss, act01_out_tensor, act02_out_tensor])
        ...
        paddle_test_act01_out, paddle_test_act02_out = exe.run(compiled_eval_prog, feed={"input_1": test_data, }, fetch_list=eval_fetch_list)
        # 自己网络的test
        print('\ntest_forward:')
        my_test_fc01_out = fc01.test_forward(test_data)
        my_test_act01_out = act01.test_forward(my_test_fc01_out)
        my_test_fc02_out = fc02.test_forward(my_test_act01_out)
        my_test_act02_out = act02.test_forward(my_test_fc02_out)
        diff_test_act01_out = np.sum((paddle_test_act01_out - my_test_act01_out)**2)
        print('diff_test_act01_out=%.6f' % diff_test_act01_out)   # 若是0，则表示成功模拟出推理
