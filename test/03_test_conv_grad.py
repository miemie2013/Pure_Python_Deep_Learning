...
if __name__ == '__main__':
    ...
    #  纯python搭建的神经网络
    conv01 = Conv2D(3, num_filters=7, filter_size=1, stride=1, padding=0, use_bias=True, name='conv01')
    conv02 = Conv2D(7, num_filters=8, filter_size=4, stride=2, padding=1, use_bias=True, name='conv02')
    mse01 = MSELoss()
    ...
    for step in range(8):
        ...
        y_true_arr = np.random.normal(loc=0, scale=1, size=(2, 8, 14, 14)).astype(np.float32)

        paddle_mseloss_out, paddle_conv01_out, paddle_conv02_out, grad222 = exe.run(train_prog, feed={"input_1": batch_data, "y_true": y_true_arr, },
                                                                       fetch_list=[mseloss, conv01_out_tensor, conv02_out_tensor, grad])
        ...
        # 求卷积层conv02的输出对其输入的偏导数。输出先对自己求偏导得1
        dy = np.ones(my_conv02_out_grad.shape)
        grad22 = conv02.get_grad(dy, optimizer2)
        diff_grad = np.sum((grad22 - grad222)**2)
        print('diff_grad=%.6f' % diff_grad)
